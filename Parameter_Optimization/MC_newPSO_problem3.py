...
def calculate_theta(eng, final_state):
    m = 1000
    n_i = 15
    theta_i = 2 * np.pi * np.random.random((m, 5))
    ...
    for i in range(m):
        pro[i] = run(eng, theta_i[i], final_state)
    #选取里面结果最好
    print(max(pro))
    for i in range(n_i):
        index = pro.index(max(pro))
        pro.remove(max(pro))
        theta[i] = theta_i[index]
    '''theta.append([4.67345,5.4132,1.592096,4.975939,5.439311])
    theta.append([4.72816395,4.13612161,4.65180772,2.25755319,4.96200525])
    theta.append([ 2.9706074,3.45605731,3.40125491,3.39205966,-0.05104171])
    theta.append([4.723114444326273,5.456658879752021,1.5720429519954697,5.032961079239871,5.268343451365208])  0.22'''
    ...
    n=len(theta)
    pro = [0] * n
    print("The Second step with p=0.5:", '\n\n')
...
'''#手动的调整步进。比不上上面的自动的调整步进。
    ...
    for i in range(5):
        ...
        max_pro = max(pro)
        location = pro.index(max_pro)
        print(max_pro, location, theta[location])
        for j in range(n):
            theta[j] = [p * x + (1 - p) * y for x, y in zip(theta[j], theta[location])]
        if max_pro >= 0.018:
            break
    ...
    p = 0.9
    print("The Second step with p=0.9:", '\n\n')
    for i in range(15):
        for j in range(len(theta)):
            pro[j] = run(eng, theta[j], final_state)
        max_pro = max(pro)
    ...
    p = 0.95
    print("The Second way with p=0.95:", '\n\n')
    for i in range(200):
        for j in range(len(theta)):
            pro[j] = run(eng, theta[j], final_state)
        max_pro = max(pro)
